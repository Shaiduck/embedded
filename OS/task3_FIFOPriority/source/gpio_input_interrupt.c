...
void SysTick_Handler(void)
{
    ...
    if (priority > MAX_VALUE_PRIORITY)
    {
        priority = 0;
    }
    taskID++;
...
void create(function_pointer_int cb, int taskID, int priority);
void del();
void display();
...
void display()
{
	...
	else
	{
        ...
        for (int maxPriority = MAX_VALUE_PRIORITY; maxPriority >= 0; maxPriority-- )
        {
            do
            {
                if (temp->priority == maxPriority)
                {
                    temp->callback(temp->taskID, temp->priority);
                }
                temp = temp->next;
            } while (temp != front);
            temp = front;
...
void testCallback(int taskNumber, int taskPrio);
...
void testCallback(int taskNumber, int taskPrio)
{
    PRINTF("\r\nThis is the task %i of priority %i and it will last %i seconds\r\n", taskNumber, taskPrio, 1);
...
void sm_StateAdd(int param)
{
    ...
    if (pendingAdds > 0)
    {
        ...
        {
            PRINTF("\r\n adding task\r\n");
            create(testCallback, taskID, priority);
        }
        pendingAdds = 0;
    }
    if (param == 2)
